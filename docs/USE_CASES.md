...
- **Who:** the user, back at the desk after a weekend.
...
- **Who:** the user, talking to Faustus with a 27B local model and a small
  context window.
- **Goal:** a two-sentence resume context for yesterday's last work, in
  Spanish, with the file he was in.
...
- **Who:** the user on Friday afternoon, preparing a weekly note.
...
- **Who:** the user, mid-task, remembering a page but not its URL.
- **Goal:** find when (and in which tab title) he saw a documentation page,
  then what he was doing around it.
...
- **Who:** the user before a video interview and a bank transfer.
- **Goal:** nothing about the interview window, the bank page or a private
  browser window is stored; afterwards, delete the half hour he forgot to
  pause.
- **Steps (UI):**
...
- **Goal:** commits from his active projects show up quickly; the Projects
  list shows the projects he actually spends time on, not every repo name
...
- **Done when:** adding the folder gives feedback (it exists, how many repos
  ...
  minutes and never blocks recording, and the Projects list contains the
  owner's projects only.

## UC7 - Coming back after a long time away

- **Who:** the user starts the app (or the PC wakes up and Faustus starts
...
- **Goal:** those 50 minutes show as "away", the first active span starts
  when he actually touches the keyboard, and nothing overlaps the previous
  session.
...
- **Who:** the user wants a chart of hours per project per day for his
  portfolio write-up.
