...
| Search | SQLite FTS5 over titles/files/commit subjects, with snippet highlighting | Falls back to substring `LIKE` search if the platform's `sqlite3` build lacks FTS5 (checked at startup) -- the Linux build used for development has FTS5; **not verified on the Windows 3.13 python.org build**, see Windows risk below |
